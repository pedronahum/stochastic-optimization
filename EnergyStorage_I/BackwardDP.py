...
class BDP:
    ...
    def state_transition(self, state, decision, exog_info):
        ...
        new_energy = (
            state.energy_amount
            + (self.model.init_args["eta"] * decision.buy)
            - decision.sell
        )
        ...
        if len(state) == 2:
            new_state = self.model.build_state(
                {"energy_amount": adjusted_new_energy, "price": adjusted_new_price}
            )
            ...
            new_state = self.model.build_state(
                {
                    "energy_amount": adjusted_new_energy,
        ...
        if len(self.model.state_variable) == 2:
            for price in self.discrete_prices:
                for energy in self.discrete_energy:
                    state = self.model.build_state(
                        {"energy_amount": energy, "price": price}
                    )
            ...
            for p in self.discrete_prices:
                for prev_p in self.discrete_prices:
                    for energy in self.discrete_energy:
                        state = self.model.build_state(
                            {"energy_amount": energy, "price": p, "prev_price": prev_p}
                        )
                        self.possible_states.append(state)

        print(
            "State dimension: {}. State space size: {}. Exogenous info size: {}".format(
                len(self.model.state_variable),
                len(self.possible_states),
                len(self.discrete_price_changes),
            )
        )
        ...
        while time != -1:
            ...
            for state in self.possible_states:
                ...
                for d in self.model.possible_decisions:
                    ...
                    contribution = price * (
                        self.model.init_args["eta"] * x.sell - x.buy
                    )
                    sum_w = 0
                    w_index = 0
                    for w in self.discrete_price_changes:
                        f = (
                            self.f_p[w_index]
                            if w_index == 0
                            else self.f_p[w_index] - self.f_p[w_index - 1]
                        )
                        next_state = self.state_transition(state, x, w)
                        next_v = (
                            values[time + 1][next_state]
                            if time < self.time

...
class ClinicalTrialsPolicy:
    ...
    def model_A_policy(self, state, info_tuple):
        ...
        stop_A = info_tuple[1]
        sim_model = ClinicalTrialsModel(
            self.model.state_variables,
            self.model.decision_variables,
    ...
    def model_C_extension_policy(self, state, info_tuple):
        ...
        if stop_C_extension == False:
            value_dict = {}
            sol_dict, value_dict = model_C_extension_value_fn(
                sim_model, 0, success_C_extension, value_dict
            )
            new_decision = sol_dict["optimal_enroll"]
        else:
            new_decision = 0
    ...
    def model_C_policy(self, state, info_tuple, time):
        ...
        if stop_C == True:
            ...
            vals = []
            decs = []
            for x_enroll in range(
                self.model.initial_state["enroll_min"],
                self.model.initial_state["enroll_max"]
                + self.model.initial_state["enroll_step"],
                self.model.initial_state["enroll_step"],
            ):
                pseudo_state = [
                    state.potential_pop + x_enroll,
                    state.success,
                    state.failure,
                    state.l_response,
                ]
                ...
                cost = -(
                    self.model.initial_state["program_cost"]
                    + self.model.initial_state["patient_cost"] * x_enroll
                )
    ...
    def run_policy(self, policy_info, policy, t):
        ...
        while (
            t <= model_copy.initial_state["trial_size"]
            and policy_info[policy][1] == False
        ):
            time_t = time.time()
            ...
            p_true_samples = np.random.beta(
                model_copy.state.success,
                model_copy.state.failure,
                model_copy.initial_state["K"],
            )
            p_belief = model_copy.state.success / (
                model_copy.state.success + model_copy.state.failure
            )
            ...
            if policy == "model_A":
                ...
                decision["enroll"] = self.model_C_extension_policy(
                    model_copy.state, p.model_C_extension
                )
            elif policy == "model_C":
            ...
            print(
                "Base Model t={}, obj={:,}, state.potential_pop={}, state.success={}, state.failure={}, x={}, elapsed time={:.2f} sec".format(
                    t,
            ...
            t += 1
        print(
            "Base Model: Stopping time t={}, obj(revenue)={:,}, rhobar={:.2f}, Elapsed time={:.2f} sec".format(
                t,
                model_copy.objective,
                model_copy.state.success
                / (model_copy.state.success + model_copy.state.failure),
...
def model_A_value_fn(model, iteration, success_index, value_dict):
    ...
    if success_index == -1:
        if iteration < model.initial_state["H"]:
            ...
            for x_enroll in range(
                ...
                model.initial_state["enroll_max"] + model.initial_state["enroll_step"],
                ...
                bellman_cost = -(
                    model.initial_state["program_cost"]
                    + model.initial_state["patient_cost"] * x_enroll
                )
                bellman_state = copy.deepcopy(model.initial_state)
                bellman_state["potential_pop"] = bellman_potential_pop
                bellman_M = ClinicalTrialsModel(
                    model.state_variables, model.decision_variables, bellman_state, True
                )
                ...
                bellman_p_belief = bellman_M.state.success / (
                    bellman_M.state.success + bellman_M.state.failure
                )
                if bellman_p_belief > bellman_M.initial_state["theta_stop_high"]:
                    ...
                    if value_key in value_dict:
                        ...
                        sol_dict, value_dict = model_A_value_fn(
                            bellman_M, iteration + 1, success_index, value_dict
                        )
    ...
    # stops experiment at node if drug is declared success or failure
        ...
        return {
            "value": model.initial_state["success_rev"] * success_index,
            "optimal_enroll": 0,
        }, value_dict
    ...
    if success_index == -1:
        if iteration < model.initial_state["H"]:
            ...
            for x_enroll in range(
                ...
                bellman_potential_pop = model.state.potential_pop + x_enroll
                bellman_enrollments = math.floor(
                    model.state.l_response * bellman_potential_pop
                )
                bellman_cost = -(
                    model.initial_state["program_cost"]
                    + model.initial_state["patient_cost"] * x_enroll
                ...
                for set_succ in range(0, bellman_enrollments, step_succ):
                    ...
                    bellman_state["failure"] = model.state.failure + (
                        bellman_enrollments - set_succ
                    )
                    bellman_M = ClinicalTrialsModel(
                    ...
                    value_key = (
                        iteration + 1,
                        bellman_state["potential_pop"],
                        bellman_state["success"],
                        bellman_state["failure"],
                    )
                    count = -1

                    # implements sampled distribution for bellman_p_true
                    bellman_p_samples = np.random.beta(
                        bellman_M.state.success,
                        bellman_M.state.failure,
                        bellman_M.initial_state["K"],
                    )
                    bellman_p_belief = bellman_M.state.success / (
                        bellman_M.state.success + bellman_M.state.failure
                    )
                    ...
                    if bellman_p_belief > bellman_M.initial_state["theta_stop_high"]:
                        ...
                        if value_key in value_dict:
                            ...
                            sol_dict, value_dict = model_B_value_fn(
                                bellman_M, iteration + 1, success_index, value_dict
                            )
                            step_value = sol_dict["value"]
                    ...
                    for k in range(0, bellman_M.initial_state["K"]):
                        bellman_cost += (
                            binom.pmf(
                                set_succ, bellman_enrollments, bellman_p_samples[k]
                            )
                            * 1
                            / bellman_M.initial_state["K"]
                            * step_value
            ...
            for x_enroll in range(
                ...
                for set_pop in range(0, x_enroll, step_pop):
                    ...
                    for set_succ in range(0, set_pop, step_succ):
                        ...
                        bellman_state["l_response"] = (
                            1 - model.initial_state["alpha"]
                        ) * model.state.l_response + model.initial_state[
                            "alpha"
                        ] * set_pop / bellman_potential_pop
                        bellman_M = ClinicalTrialsModel(
                            model.state_variables,
                            model.decision_variables,
                            bellman_state,
                        ...
                        if (
                            bellman_p_belief
                            > bellman_M.initial_state["theta_stop_high"]
                        ):
                            success_index = 1
                            step_value = model.initial_state["success_rev"]
                        elif (
                            bellman_p_belief < bellman_M.initial_state["theta_stop_low"]
                        ):
                            success_index = 0
                            step_value = 0
                        ...
                        for k in range(0, bellman_M.initial_state["K"]):
                            bellman_cost += (
                                binom.pmf(set_succ, set_pop, bellman_p_samples[k])
                                * trunc_probs[set_pop]
                                * 1
                                / bellman_M.initial_state["K"]
                                * step_value
...
def parameters_fn(model):
    ...
    for n in range(model.initial_state["sampling_size"]):
        sample_t = 0
        stop = False
        sample_M = ClinicalTrialsModel(
            model.state_variables, model.decision_variables, model.initial_state, True
        )
        while sample_t <= model.initial_state["trial_size"] and stop == False:
            p_belief = sample_M.state.success / (
                sample_M.state.success + sample_M.state.failure
            )
            p_true_samples = np.random.beta(
                sample_M.state.success,
                sample_M.state.failure,
                sample_M.initial_state["K"],
            )
            ...
            pseudo_state = [
                sample_M.state.potential_pop + decision["enroll"],
                sample_M.state.success,
                sample_M.state.failure,
                sample_M.state.l_response,
            ]
            sample_M.step(x)
            sample_t += 1
        ...
        if t_fct == 0:
            ...
            parameters.append(
                np.matrix.tolist(
                    scipy.optimize.curve_fit(
                        func_simple, np.matrix.transpose(samples_array), values_array
                    )[0]
                )
            )
        else:
            if len(values_array) >= 8:
                parameters.append(
                    np.matrix.tolist(
                        scipy.optimize.curve_fit(
                            func, np.matrix.transpose(samples_array), values_array
                        )[0]
                    )
                )
            else:
                parameters.append(
                    np.matrix.tolist(
                        scipy.optimize.curve_fit(
                            func_simple,
                            np.matrix.transpose(samples_array),
                            values_array,
                        )[0]
                    )
                )

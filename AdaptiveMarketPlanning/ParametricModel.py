...
class ParametricModel(AdaptiveMarketPlanningModel):
    ...
    def transition_fn(self, decision, exog_info):
        ...
        if (
            self.order_quantity_fn(self.state.price, self.state.theta)
            < exog_info["demand"]
        ):
            derivative = (self.state.price - self.cost) * self.derivative_fn(
                self.state.price, self.state.theta
            )
        else:
            derivative = (-self.cost) * self.derivative_fn(
                self.state.price, self.state.theta
            )
        ...
        new_counter = (
            ...
            if np.dot(self.past_derivative, derivative) < 0
            else self.state.counter
        )

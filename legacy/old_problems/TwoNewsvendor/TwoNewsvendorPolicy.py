...
from TwoNewsvendor import Model_Central, Model_Field
...
def plot_heat_map(
    ...
    params,
    theta_field_values,
    theta_central_values,
    titleString,
    player_sorted_by_value,
):
    """
    ...
    contribution_values = [
        contribution_dict[(theta_field, theta_central)]
        for theta_central in theta_central_values
        for theta_field in theta_field_values
    ]
    ...
    im = ax.imshow(contributions, cmap="hot", origin="lower", aspect="auto", alpha=0.9)
    ...
    for rank_player, dict_entry_player in enumerate(player_sorted_by_value):
        ...
        y_ind = [
            i
            for i, y in enumerate(theta_central_values)
            if dict_entry_player[0][1] == y
        ]
        x_ind = [
            i for i, x in enumerate(theta_field_values) if dict_entry_player[0][0] == x
        ]

        text = ax.text(
            x_ind[0],
            y_ind[0],
            "{}\n {:.0f}".format(rank_player, dict_entry_player[1][-1]),
            ha="center",
            va="center",
            color=textcolors[im.norm(dict_entry_player[1][-1]) < threshold],
            fontsize=7,
        )
...
def run_policies(
    ...
    record_sample_ite = [
        ...
        "{}_{}".format(theta_field, theta_central),
        ite,
    ]
    ...
    for n in range(params["N"]):
        ...
        record_sample_t = [
            n,
            exog_info_gen.get_Round_Number(),
            exog_info_gen.get_Estimate_Field(),
            exog_info_gen.get_Estimate_Central(),
            exog_info_gen.get_Demand(),
        ]
        ...
        M_field.build_decision(
            {"quantity_requested": field_request, "bias_applied": bias_field}
        )
        ...
        M_central.build_decision(
            {"quantity_allocated": decision_central, "bias_applied": bias_central}
        )
    ...
    return (
        ...
        cost_ite_central,
        record_budget,
        accum_request_field / params["N"],
        accum_allocated_central / params["N"],
    )
...
class Policy_Field:
    ...
    def getLearningBias(self, model):
        ...
        if "UCB" in model.init_args["policy_field"]:
            stats = {
                x: model.choices[x].get_UCB_value(model.n + 1)
                for x in model.choice_range
            }
        else:
    ...
    def regular(self, model):
        ...
        decision = round(
            ...
            - model.state.source_bias
            - model.state.central_bias
            + self.theta
        )
...
class Policy_Central:
    ...
    def regular(self, model):
        ...
        decision = round(
            model.state.field_request - model.state.field_bias + self.theta
        )
    ...
    def learning_IE_two_estimates(self, model):
        bias = self.getLearningBias(model)
        decision = round(
            model.state.field_weight * (model.state.field_request)
            + model.state.source_weight
            * (model.state.estimate - model.state.source_bias)
            + bias
        )

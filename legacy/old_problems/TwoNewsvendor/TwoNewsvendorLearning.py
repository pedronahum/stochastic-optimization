...
class Choice:
    def __init__(
        self, quantity, util_estimate, W_precision_estimate, theta, nu_bar=0.5
    ):
        """
    ...
    def upload_results(self, W):
        ...
        self.util_estimate = (
            self.util_estimate * self.accumulated_precision + W * self.W_precision
    ...
    def get_UCB_value(self, time):
        if self.n == 0:
            ...
            UCB_val = self.util_estimate + self.theta * math.sqrt(
                math.log(time) / self.n
            )
        return UCB_val

    def get_IE_value(self):

        IE_val = self.util_estimate + self.theta * math.sqrt(
    ...
    def getAllParametersList(self, time):
        outL = "{:.2f} {} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} ".format(
            self.quantity,
            self.n,
            self.util_estimate,
            self.accumulated_precision,
            math.sqrt(1 / self.accumulated_precision),
            self.get_IE_value(),
            self.get_UCB_value(time),
            self.nu,
            self.W_beta,
            self.W_delta,
            self.W_variance,
...
class Learning_model_field(Model_Field):
    ...
    def getMainParametersHeaderList(self):
        listPar = [
            self.choices[x].getMainParametersHeaderList() for x in self.choice_range
        ]
        listParFlat = [
            str(x) + "_field_" + elem
    ...
    def getMainParametersDf(self):
        dictPar = {
            x: self.choices[x].getMainParametersList() for x in self.choice_range
        }

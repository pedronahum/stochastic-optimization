...
class Exogenous_Information:
    ...
    def generate_New_Round(self):
        ...
        self.demand = int(
            round(
                self.prng.uniform(
                    self.init_args["dem_lower_bound"], self.init_args["dem_upper_bound"]
                )
            )
        )

        self.estimate_field = max(
            0,
            int(
                round(
                    self.demand
                    + self.prng.normal(
                        loc=self.init_args["est_bias_field"],
                        scale=self.init_args["est_std_field"],
                    )
                )
            ),
        )
        self.estimate_central = max(
            0,
            int(
                round(
                    self.demand
                    + self.prng.normal(
                        loc=self.init_args["est_bias_central"],
                        scale=self.init_args["est_std_central"],
                    )
                )
            ),
        )
...
class Model_Field:
    ...
    def transition_fn(self, exog_info):
        ...
        central_bias = (
            exog_info["allocated_quantity"] - self.decision.quantity_requested
        )

        for state_desc in ["central_bias", "source_bias"]:
            state_dict[state_desc] = (1 - self.get_alpha_bias()) * state_dict[
                state_desc
            ] + self.get_alpha_bias() * eval(state_desc)
    ...
    def objective_fn(self, exog_info):
        ...
        self.pen_incurred = self.init_args["o_field"] * max(
            allocated - demand, 0
...
class Model_Central:
    ...
    def transition_fn(self, exog_info):
        ...
        self.beta_field = (
            1 - self.get_alpha_learning()
        ) * self.beta_field + self.get_alpha_learning() * (
            field_bias - state_dict["field_bias"]
        )
        self.beta_source = (
            1 - self.get_alpha_learning()
        ) * self.beta_source + self.get_alpha_learning() * (
            source_bias - state_dict["source_bias"]
        )

        self.delta_field = (
            1 - self.get_alpha_learning()
        ) * self.delta_field + self.get_alpha_learning() * (
            (field_bias - state_dict["field_bias"]) ** 2
        )
        self.delta_source = (
            1 - self.get_alpha_learning()
        ) * self.delta_source + self.get_alpha_learning() * (
            (source_bias - state_dict["source_bias"]) ** 2
        )

        self.var_field = (self.delta_field - (self.beta_field**2)) / (
            1 - self.lambda_field
        )
        self.var_source = (self.delta_source - (self.beta_source**2)) / (
            1 - self.lambda_source
        )
        ...
        if self.n > 1:
            self.lambda_field = (
                (1 - self.get_alpha_bias()) ** 2
            ) * self.lambda_field + self.get_alpha_bias() ** 2
            self.lambda_source = (
                (1 - self.get_alpha_bias()) ** 2
            ) * self.lambda_source + self.get_alpha_bias() ** 2
        else:
    ...
    def objective_fn(self, exog_info):
        allocated = exog_info["allocated_quantity"]
        demand = exog_info["demand"]
        self.pen_incurred = self.init_args["o_central"] * max(
            allocated - demand, 0

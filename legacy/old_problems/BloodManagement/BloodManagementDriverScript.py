...
from BloodManagementPolicy import Policy, initLPMatrices
...
def loadParams(filename):
    ...
    params["NUM_DEM_NODES"] = (
        params["NUM_BLD_TYPES"] * params["NUM_SUR_TYPES"] * len(params["Substitution"])
    )
    ...
    params["NUM_ITER"] = int(
        params["NUM_TESTING_ITER"] + params["NUM_TRAINNING_ITER"]
    ...
    params["LOAD_VFA"] = (
        False  # If set to True we are going to initialize the VFA's with VFA's from previous runs - instead of all zeros
    )
    params["NAME_LOAD_VFA_PICKLE"] = "Bld_Net10_P_C_Subs.pickle"
    params["SAVE_VFA"] = (
        False  # If we want to save/update the VFA's to be used in future runs
    )
    params["NAME_SAVE_VFA_PICKLE"] = "Bld_Net10_P_C_Subs.pickle"

    # Set here the stepsize parameters
    params["STEPSIZE_RULE"] = (
        "C"  # Possible values: 'C' for Constant or 'A' for AdaGrad
    )
    ...
    params["DISCARD_BLOOD_PENALTY"] = (
        -10
    )  # applied for the oldest age in the holding/vfa arcs
    ...
    # Default
    params["SURGERYTYPES_PROP"] = {
        k: 1 / len(params["Surgerytypes"]) for k in params["Surgerytypes"]
    }
    params["SUBSTITUTION_PROP"] = {
        k: 1 / len(params["Substitution"]) for k in params["Substitution"]
    }
    ...
    # SURGE_PROB = 0.7
    params["SURGE_FACTOR"] = (
        6  # The surge demand is always going to be poisson with mean SURGE_FACTOR*params['MAX_DEM_BY_BLOOD'], even if the regular demand distribution is Uniform
    )
    ...
    if params["SAMPLING_DIST"] == "P":
        params["MAX_DEM_BY_BLOOD"] = {
            k: int(v * params["POISSON_FACTOR"])
            for k, v in params["MAX_DEM_BY_BLOOD"].items()
        }
        params["MAX_DON_BY_BLOOD"] = {
            k: int(v * params["POISSON_FACTOR"])
            for k, v in params["MAX_DON_BY_BLOOD"].items()
        }

        params["AVG_TOTAL_DEMAND"] = sum(params["MAX_DEM_BY_BLOOD"].values())
        params["AVG_TOTAL_SUPPLY"] = sum(params["MAX_DON_BY_BLOOD"].values())
        params["NUM_PARALLEL_LINKS"] = int(
            params["MAX_AGE"] / 2 * max(params["MAX_DON_BY_BLOOD"].values())
        )
    ...
    print(
        "There are ",
        params["NUM_SUR_TYPES"] * len(params["Substitution"]),
        " demand nodes for each blood type",
    )
...
def convertToDfOutputLists(
    ...
    (
        ...
        headerSimuList,
        headerUpdateVfaList,
    ) = initOutputListHeaders(params)

    # Flatteting the lists
    dfSimu = pd.DataFrame.from_records(simuList, columns=headerSimuList)

    demandExoListFlat = [
        (ite, t, dnode[0], dnode[1], dnode[2], dvalue)
        for ite, t, d in demandExoList
        for dnode, dvalue in zip(Bld_Net.demandnodes, d)
    ]
    dfDemandExo = pd.DataFrame.from_records(demandExoListFlat, columns=labelsDemandExo)

    donationExoListFlat = [
        (ite, t, dtype, dvalue)
        for ite, t, d in donationExoList
        for dtype, dvalue in zip(params["Bloodtypes"], d)
    ]
    dfDonationExo = pd.DataFrame.from_records(
        donationExoListFlat, columns=labelsDonationExo
    )

    supplyPreListFlat = [
        (ite, t, bnode[0], bnode[1], bvalue)
        for ite, t, b in supplyPreList
        for bnode, bvalue in zip(Bld_Net.bloodnodes, b)
    ]
    dfSupplyPre = pd.DataFrame.from_records(supplyPreListFlat, columns=labelsSupplyPre)

    supplyPostListFlat = [
        (ite, t, bnode[0], bnode[1], bvalue)
        for ite, t, b in supplyPostList
        for bnode, bvalue in zip(Bld_Net.bloodnodes, b)
    ]
    dfSupplyPost = pd.DataFrame.from_records(
        supplyPostListFlat, columns=labelsSupplyPost
    )
    ...
    solDemListFlat = [
        (
            ...
            dem[1],
            dem[2],
            params["SubMatrix"][(bld[0], dem[0])],
            Bld_Net.demweights[(bld, dem)],
            xbd,
        )
        for ite, t, xDem in solDemList
        for bld, xb in zip(Bld_Net.bloodnodes, xDem)
        for dem, xbd in zip(Bld_Net.demandnodes, xb)
    ]
    dfSolDem = pd.DataFrame.from_records(solDemListFlat, columns=headerSolDemList)

    solHoldListFlat = [
        (ite, t, bnode[0], bnode[1], hvalue)
        for ite, t, h in solHoldList
        for bnode, hvalue in zip(Bld_Net.bloodnodes, h)
    ]
    dfSolHold = pd.DataFrame.from_records(solHoldListFlat, columns=headerSolHoldList)

    slopesListFlat = [
        (vnode[0], vnode[1], bnode[0], bnode[1], *list(vnode[2]))
        for vnode, bnode in zip(
            slopesList, Bld_Net.bloodnodes * params["NUM_ITER"] * params["MAX_TIME"]
        )
    ]
...
def Main():
    ...
    while iteration < params["NUM_ITER"]:
        ...
        if params["SAMPLING_DIST"] == "P":
            bldinv_init = [
                (
                    int(np.random.poisson(params["MAX_DON_BY_BLOOD"][bld[0]]) * 0.9)
                    if bld[1] == "0"
                    else int(
                        np.random.poisson(params["MAX_DON_BY_BLOOD"][bld[0]])
                        * (0.1 / (params["MAX_AGE"] - 1))
                    )
                )
                for bld in Bld_Net.bloodnodes
            ]
        else:
            bldinv_init = [
                (
                    round(
                        np.random.uniform(0, params["MAX_DON_BY_BLOOD"][bld[0]]) * 0.9
                    )
                    if bld[1] == "0"
                    else round(
                        np.random.uniform(0, params["MAX_DON_BY_BLOOD"][bld[0]])
                        * (0.1 / (params["MAX_AGE"] - 1))
                    )
                )
                for bld in Bld_Net.bloodnodes
        ...
        init_state = {
            "BloodInventory": bldinv_init,
            "Demand": exog_info_init.demand,
            "Donation": exog_info_init.donation,
        }
        ...
        while t < params["MAX_TIME"]:
            ...
            sol, val, x, hld, d, solDemList, solHoldList = P.getLPSol(
                params, M, iteration, t, solDemList, solHoldList, IS_TRAINING
            )
            obj[iteration] += val
            ...
            if t == 0:
                recordDonationExo = (
                    iteration,
                    0,
                    list(np.array(M.bld_inv)[:: params["MAX_AGE"]]),
                )
            ...
            if IS_TRAINING:
                alpha, slopesList, updateVfaList = P.updateVFAs(
                    params, M, iteration, t, d, slopesList, updateVfaList
                )
        ...
        recordSimu = (
            ...
            alpha,
            obj[iteration],
            (iteration < params["NUM_TRAINNING_ITER"]),
        )
        simuList.append(recordSimu)

        print(
            "***Finishing iteration {} in {:.2f} secs. Total contribution: {:.2f}***\n".format(
                recordSimu[0], recordSimu[1], recordSimu[3]
            )
        )
    ...
    surge = (
        params["SURGE_PROB"] > 0 and "SURGE_" + str(params["SURGE_PROB"]) or "NO_SURGE"
    )
    instance = "Policy{}_{}_PEN_{:,}_ALPHA_{:.2f}".format(
        policy, surge, params["BLOOD_FOR_ELECTIVE_PENALTY"], params["ALPHA"]
    )
    ...
    totalDiscarded = (
        dfSolHold.loc[
            (dfSolHold.Age.astype(int) > params["MAX_AGE"] - 2)
            & (dfSolHold.Iteration >= params["NUM_TRAINNING_ITER"]),
            :,
        ]
        .copy()["Value"]
        .sum()
    ...
    # Total Donation
    totalDonation = (
        dfDonationExo[dfDonationExo.Iteration >= params["NUM_TRAINNING_ITER"]]
        .copy()["DonationValue"]
        .sum()
    )

    # Coverage
    dfCoverage = dfSolDem.groupby(["BloodTypeD", "Urgency", "Iteration", "Time"])[
        "Value"
    ].sum()
    dfCoverage.index = dfCoverage.index.rename("Bloodtype", level=0)
    dfCoverage = pd.concat(
        [
            dfCoverage,
            dfDemandExo.groupby(["Bloodtype", "Urgency", "Iteration", "Time"])[
                "DemandValue"
            ].sum(),
        ],
        axis=1,
    ...
    dfCoverage_agg_ite = (
        dfCoverage.groupby(["Bloodtype", "Urgency", "Iteration"])["Ratio"]
        .mean()
        .reset_index()
    )
    ...
    dfPrintIte = dfCoverage_agg_test.pivot_table(
        "Ratio", index="Bloodtype", columns="Urgency"
    )

    finalCoverage = dfCoverage_agg_test.groupby("Urgency")["Ratio"].mean()

    coverage = (
        "Average Coverage: -  Urgent: {:.2f} Elective: {:.2f} Avg: {:.2f}".format(
            finalCoverage["Urgent"],
            finalCoverage["Elective"],
            dfCoverage_agg_ite["Ratio"].mean(),
        )
    )
    ...
    # Utility function
    utility = (
        params["WEIGHT_URGENT"] * round(finalCoverage["Urgent"], 2)
        + params["WEIGHT_ELECTIVE"] * round(finalCoverage["Elective"], 2)
    ) * 100
    modifiedUtil = utility - params["WEIGHT_DISCARDED"] * 100 * round(
        totalDiscarded / totalDonation, 2
    )
    ...
    ax_ite.set_title(
        "Policy {}_{} - Total contributions \n Avg total contribution during TESTING iterations: ${:,}\n Final utility: {:.0f}".format(
            policy, surge, meanTesting, modifiedUtil
        )
    )
    ax_ite.set_xticks(ite)
    ax_ite.set_xticklabels(
        list(ite_TRA) + list(np.arange(0, params["NUM_TESTING_ITER"], 1))
    )
    for c in ite_TES:
    ...
    dfDemandExoP = dfDemandExo[
        dfDemandExo.Iteration >= params["NUM_TRAINNING_ITER"]
    ].copy()
    dfDemandExoP["Iteration"] = dfDemandExoP["Iteration"] - params["NUM_TRAINNING_ITER"]
    dfPrintDemand = dfDemandExoP.pivot_table(
        "DemandValue", index="Time", columns="Iteration", aggfunc="sum"
    )
    l = dfPrintDemand.plot(
        ax=ax_exo[0], title="Total Demand - {}".format(surge), legend=False
    )

    dfDonationExoP = dfDonationExo[
        dfDonationExo.Iteration >= params["NUM_TRAINNING_ITER"]
    ].copy()
    dfDonationExoP["Iteration"] = (
        dfDonationExoP["Iteration"] - params["NUM_TRAINNING_ITER"]
    )
    dfPrintDonation = dfDonationExoP.pivot_table(
        "DonationValue", index="Time", columns="Iteration", aggfunc="sum"
    )
    ...
    fig_exo.legend(
        l,
        labels=list(np.arange(0, params["NUM_TESTING_ITER"], 1)),
        title="Iteration",
        loc="center right",
        fancybox=True,
        shadow=True,
    ...
    for age in [0, 1, 2]:
        ...
        dfPrint = dfInv[dfInv.Age == strage].pivot_table(
            "PreInv", index=["Time"], columns=["Iteration"], aggfunc="sum"
        )
        l = dfPrint.plot(ax=ax_inv[age], legend=False)

        dfPrintAvg = (
            dfInv[dfInv.Age == strage].groupby(["Time", "Iteration"])["PreInv"].sum()
        )
        dfPrintAvg = dfPrintAvg.groupby("Time").mean()
        avg_line = ax_inv[age].plot(
            dfPrintAvg.index,
            dfPrintAvg.values,
            "k",
            linestyle=":",
        ...
        ax_inv[age].set_title(
            "Age: {} - Avg inventory level: {:.0f}".format(
                age, dfPrintAvg.values.mean()
            )
        )
    ...
    fig_inv.suptitle(
        "Policy {}_{} \n Pre-decision  inventory level all blood types".format(
            policy, surge
        )
    )
    fig_inv.legend(
        [l, avg_line],
        labels=list(np.arange(0, params["NUM_TESTING_ITER"], 1)) + ["Avg"],
        title="Iteration",
        loc="center right",
        fancybox=True,
        shadow=True,
    ...
    for m, b in enumerate(params["Bloodtypes"]):
        ...
        dfPrint = dfInv[dfInv.BloodType == b].pivot_table(
            "PreInv", index=["Time"], columns=["Iteration"], aggfunc="sum"
        )
        l = dfPrint.plot(ax=ax_inv_blood[row, col], legend=False)

        dfPrintAvg = (
            dfInv[dfInv.BloodType == b].groupby(["Time", "Iteration"])["PreInv"].sum()
        )
        dfPrintAvg = dfPrintAvg.groupby("Time").mean()

        avg_line = ax_inv_blood[row, col].plot(
            dfPrintAvg.index,
            dfPrintAvg.values,
            "k",
            linestyle=":",
        ...
        ax_inv_blood[row, col].set_title(
            "Bloodtype: {}  - Avg inventory level: {:.0f}".format(
                b, dfPrintAvg.values.mean()
            ),
            fontsize=8,
        )
        ax_inv_blood[row, col].set_xlabel("Time Period", fontsize=8)

    fig_inv_blood.suptitle(
    ...
    fig_inv_total.suptitle(
        "Policy {}_{} \n Pre-decision inventory level all blood types and ages \n Average inventory level across time periods: {:.0f}".format(
            policy, surge, dfPrintAvg.values.mean()
        )
    )
    ###########################################################################################################################################
    ...
    dfDiscarded = dfSolHold.loc[
        (dfSolHold.Age.astype(int) > params["MAX_AGE"] - 2)
        & (dfSolHold.Iteration >= params["NUM_TRAINNING_ITER"]),
        :,
    ].copy()
    dfDiscarded = (
        dfDiscarded.groupby(["BloodTypeS", "Time"])["Value"].sum().reset_index()
    )
    ...
    im = ax_dis.imshow(
        discarded_matrix, cmap="hot_r", origin="lower", aspect="auto", alpha=0.9
    )
    ...
    ax_dis.set_title(
        "Policy {}_{} \n Total discarded blood during TESTING iterations\n Proportion of blood discarded: {:.2f}%".format(
            policy, surge, totalDiscarded * 100 / totalDonation
        )
    )
    ...
    if True:
        ...
        for i, ite in enumerate(selectedIte):
            ...
            fig_cover.suptitle(
                "Policy {}-{} \n Average coverage of demand by blood type and urgency level for different iterations".format(
                    policy, surge
                )
            )
        ...
        if len(selectedIte) == 1:
            ...
            ax_cover[len(selectedIte) - 1].legend(
                title="Urgency level",
                loc="center left",
                bbox_to_anchor=(1, 0.5),
                fancybox=True,
    ...
    if True:
        dfCoverage_agg_ite = (
            dfCoverage.groupby(["Time", "Urgency", "Iteration"])["Ratio"]
            .mean()
            .reset_index()
        )
        # selectedIte = sorted(list(set([0,5,10,19]) ))
        selectedIte = sorted(list(set([0, 5, 10, 19]) & set(iteS)))
        fig_cover_ite, ax_cover_ite = plt.subplots(
            1, len(selectedIte), figsize=(16, 8), sharey=True, sharex=True
        )
        for i, ite in enumerate(selectedIte):
            dfPrintIte = dfCoverage_agg_ite[dfCoverage_agg_ite.Iteration == ite]
            dfPrintIte = dfPrintIte.pivot_table(
                "Ratio", index="Time", columns="Urgency"
            )
            ...
            fig_cover_ite.suptitle(
                "Policy {}-{} \n Average coverage of demand by time period and urgency level for different iterations".format(
                    policy, surge
                )
            )
        ...
        if len(selectedIte) == 1:
            ...
            ax_cover_ite[len(selectedIte) - 1].legend(
                title="Urgency level",
                loc="center left",
                bbox_to_anchor=(1, 0.5),
                fancybox=True,
    ...
    uncoverU = (
        dfCoverage.loc[idx[:, ["Urgent"], ite_TES, :], ["DemandValue"]].values.sum()
        - dfCoverage.loc[idx[:, ["Urgent"], ite_TES, :], ["Value"]].values.sum()
    )
    uncoverE = (
        dfCoverage.loc[idx[:, ["Elective"], ite_TES, :], ["DemandValue"]].values.sum()
        - dfCoverage.loc[idx[:, ["Elective"], ite_TES, :], ["Value"]].values.sum()
    )
    uncoverT = (
        dfCoverage.loc[idx[:, :, ite_TES, :], ["DemandValue"]].values.sum()
        - dfCoverage.loc[idx[:, :, ite_TES, :], ["Value"]].values.sum()
    )

    demandU = dfCoverage.loc[
        idx[:, ["Urgent"], ite_TES, :], ["DemandValue"]
    ].values.sum()
    demandE = dfCoverage.loc[
        idx[:, ["Elective"], ite_TES, :], ["DemandValue"]
    ].values.sum()
    ...
    ax_hist[0].hist(
        dfCoverage.loc[idx[:, ["Urgent"], ite_TES, :], ["Ratio"]].values,
        bins=11,
        color="tab:blue",
    )
    ...
    ax_hist[0].annotate(
        "Uncovered Demand: {:,}\n Total Demand: {:,}".format(int(uncoverU), demandU),
        xy=(0.8, 0.975),
    ...
    ax_hist[1].hist(
        dfCoverage.loc[idx[:, ["Elective"], ite_TES, :], ["Ratio"]].values,
        bins=11,
        color="tab:orange",
    )
    ...
    ax_hist[1].annotate(
        "Uncovered Demand: {:,}\n Total Demand: {:,}".format(int(uncoverE), demandE),
        xy=(0.8, 0.975),
        xycoords="axes fraction",
        horizontalalignment="right",
        verticalalignment="top",
    ...
    ax_hist[2].hist(
        dfCoverage.loc[idx[:, :, ite_TES, :], ["Ratio"]].values,
        bins=11,
        color="tab:gray",
    )
    ax_hist[2].set_title("Total")
    ax_hist[2].annotate(
        "Uncovered Demand: {:,}\n Total Demand: {:,}".format(int(uncoverT), demandT),
        xy=(0.8, 0.975),
        xycoords="axes fraction",
        horizontalalignment="right",
        verticalalignment="top",
    ...
    print(instance)
    print(
        "Average total contribution during TESTING iterations: ${:,}".format(
            meanTesting
        )
    )
    print(coverage)
    print(
        "Proportion of blood discarded: {:.2f}% ".format(
            totalDiscarded * 100 / totalDonation
        )
    )

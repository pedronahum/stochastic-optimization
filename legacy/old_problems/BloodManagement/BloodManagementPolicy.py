...
def initLPMatrices(params, Bld_Net):
    ...
    A = np.zeros(
        (
            params["NUM_BLD_NODES"],
            params["NUM_BLD_NODES"]
            * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"]),
        )
    )
    for i in range(params["NUM_BLD_NODES"]):
        for j in range(
            ...
            * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"])
            ...
            if (
                j < (i + 1) * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"])
            ) and (j >= i * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"])):
                ...
                if k < params["NUM_DEM_NODES"]:
                    ...
                    if (demandnode[2] == False and bloodnode[0] == demandnode[0]) or (
                        demandnode[2] == True
                        and params["SubMatrix"][(bloodnode[0], demandnode[0])] == True
                    ):
                        A[i, j] = 1
                else:
                    A[i, j] = 1
    ...
    G = np.zeros(
        (
            ...
            + 2 * params["NUM_BLD_NODES"] * params["NUM_PARALLEL_LINKS"]
            + params["NUM_DEM_NODES"] * params["NUM_BLD_NODES"],
            params["NUM_BLD_NODES"]
            * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"]),
        )
    )
    ...
    for i in range(params["NUM_BLD_NODES"]):
        for j in range(params["NUM_PARALLEL_LINKS"]):
            G[
                params["NUM_DEM_NODES"] + i * params["NUM_PARALLEL_LINKS"] + j,
                (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"]) * i
                + params["NUM_DEM_NODES"]
                + j,
            ] = 1
    ...
    for i in range(params["NUM_BLD_NODES"]):
        for j in range(params["NUM_DEM_NODES"]):
            G[
                params["NUM_DEM_NODES"]
                + params["NUM_BLD_NODES"] * params["NUM_PARALLEL_LINKS"]
                + i * params["NUM_DEM_NODES"]
                + j,
                (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"]) * i + j,
            ] = -1
    ...
    h = np.ones(
        params["NUM_DEM_NODES"] + params["NUM_BLD_NODES"] * params["NUM_PARALLEL_LINKS"]
    )
    h[params["NUM_DEM_NODES"] :: params["NUM_PARALLEL_LINKS"]] = params[
        "SLOPE_CAPAC_LAST"
    ]
    h = np.append(
        h,
        np.zeros(
            params["NUM_BLD_NODES"] * params["NUM_DEM_NODES"]
            + params["NUM_BLD_NODES"] * params["NUM_PARALLEL_LINKS"]
        ),
    )
    ...
    coeff = [
        (
            np.concatenate(
                (
                    np.array(Bld_Net.demcontrib[bld]),
                    np.zeros(params["NUM_PARALLEL_LINKS"]),
                )
            )
            if int(bld[1]) < params["MAX_AGE"] - 1
            else np.concatenate(
                (
                    np.array(Bld_Net.demcontrib[bld]),
                    np.add(
    ...
    def getLPSol(self, params, M, iteration, t, solDemList, solHoldList, IS_TRAINING):
        ...
        c_t = [
            (
                np.concatenate(
                    (
                        np.multiply(np.array(M.Bld_Net.demcontrib[bld]), -1),
                        np.multiply(
                            M.Bld_Net.parallelarr[(t, bld, M.Bld_Net.supersink)],
            ...
            for bld in M.Bld_Net.bloodnodes
        ]
        ...
        xDem = [
            ...
            for i in list(range(params["NUM_BLD_NODES"]))
        ]
        ...
        hld = [
            np.sum(
                x[
                    i * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"])
                    + params["NUM_DEM_NODES"] : (i + 1)
                    * (params["NUM_DEM_NODES"] + params["NUM_PARALLEL_LINKS"])
                ]
            )
            for i in list(range(params["NUM_BLD_NODES"]))
        ...
        invByBlood = [
            np.sum(M.bld_inv[i * params["MAX_AGE"] : (i + 1) * params["MAX_AGE"]])
            for i in list(range(len(params["Bloodtypes"])))
        ]
        demByBlood = [
            np.sum(
                M.Bld_Net.demandamount[
                    i
                    * (len(params["Surgerytypes"]) * len(params["Substitution"])) : (
                        i + 1
                    )
                    * (len(params["Surgerytypes"]) * len(params["Substitution"]))
                ]
        ...
        xDemFlat = [xij for xi in xDem for xij in xi]
        xDemMat = np.array(xDemFlat).reshape(
            params["NUM_BLD_NODES"], params["NUM_DEM_NODES"]
        )
        ...
        hldByBlood = [
            int(np.sum(hld[i * params["MAX_AGE"] : (i + 1) * params["MAX_AGE"]]))
            for i in list(range(len(params["Bloodtypes"])))
        ]
        ...
        if IS_TRAINING and params["IS_PERTUB"]:
            ...
            signE = PERTUB_GEN.choice(
                [-1, 1], size=params["NUM_BLD_NODES"], replace=True, p=None
            )
    ...
    def updateVFAs(self, params, M, iteration, t, d, slopesList, updateVfaList):
        ...
        for i in range(params["NUM_BLD_NODES"]):
            ...
            recordSlopes = (
                iteration,
                t,
                M.Bld_Net.parallelarr[
                    (t, M.Bld_Net.holdnodes[i], M.Bld_Net.supersink)
                ].copy(),
            )
            ...
            if index >= 0:
                if t > 0 and M.Bld_Net.holdnodes[i][1] < str(params["MAX_AGE"] - 1):
                    ...
                    arr = M.Bld_Net.varr[
                        (t - 1, M.Bld_Net.holdnodes[i], M.Bld_Net.supersink)
                    ]
                    ...
                    if iteration < params["NUM_ITER_STEP_ONE"]:
                        ...
                        if params["STEPSIZE_RULE"] == "C":
                            ...
                            alpha = params["ETA"] / (
                                np.sqrt(sqGradArr[index] + params["STEP_EPS"])
                            )
                    ...
                    recordUpdateVfa = (
                        iteration,
                        t - 1,
                        M.Bld_Net.holdnodes[i][0],
                        M.Bld_Net.holdnodes[i][1],
                        index,
                        vhat,
                        vbar,
                        sqGradArr[index],
                    ...
                    if vnew > vbar:  # Look to the left
                        indSetL = [
                            i for i in list(range(0, index + 1)) if arr[i] <= vnew
                        ]
                        if len(indSetL) > 0:
                        ...
                        indSetR = [
                            i
                            for i in list(range(index, params["NUM_PARALLEL_LINKS"]))
                            if arr[i] >= vnew
                        ]
                        if len(indSetR) > 0:
                            if params["PROJECTION_ALGO"] == "Avg":

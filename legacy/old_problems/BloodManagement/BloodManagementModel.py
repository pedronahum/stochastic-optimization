...
def generate_exog_info_by_bloodtype(t, Bld_Net, params):
    ...
    demand = []
    if (
        t in params["TIME_PERIODS_SURGE"]
        and np.random.uniform(0, 1) < params["SURGE_PROB"]
    ):
    ...
    demand = [
        round(
            np.random.uniform(
                0,
                params["MAX_DEM_BY_BLOOD"][dmd[0]]
                * params["SURGERYTYPES_PROP"][dmd[1]]
    ...
    # donation
    donation = [
        round(np.random.uniform(0, params["MAX_DON_BY_BLOOD"][i]))
...
def generate_exog_info_by_bloodtype_p(t, Bld_Net, params):
            ...
            if dmd[0] == "O-":
                if dmd[1] == "Urgent":
                    ...
                    eleDem = max(
                        ...
                        int(
                            np.random.poisson(
                                factor * params["MAX_DEM_BY_BLOOD"][dmd[0]] - 1
                            )
                        )
                        - 1,
                    )
    ...
    # donation
    donation = [
        int(np.random.poisson(params["MAX_DON_BY_BLOOD"][i]))
        for i in params["Bloodtypes"]
    ]
...
def contribution(params, bloodnode, demandnode):
    ...
    if (demandnode[2] == False and bloodnode[0] != demandnode[0]) or (
        demandnode[2] == True
        and params["SubMatrix"][(bloodnode[0], demandnode[0])] == False
    ):

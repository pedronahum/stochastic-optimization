...
def mc_success_fn(count, mean, samples, N, K):
    ...
    for n in range(N):
        ...
        MC_r_sample = np.random.choice(
            range(count + 1), size=None, replace=True, p=trunc_probs
        )
...
class ClinicalTrialsModel:
    ...
    def exog_info_fn(self, decision):
        ...
        if self.simulation == False:
            exog_patients = math.floor(
                np.random.poisson(
                    lam=self.initial_state["true_l_response"]
                    * (self.state.potential_pop + decision.enroll),
                    size=None,
                )
            )
            exog_succ = math.floor(
                np.random.binomial(
                    exog_patients, self.initial_state["true_succ_rate"], size=None
                )
            )
            ...
            return {"new_patients": exog_patients, "succ_count": exog_succ}
        else:
            r_bar = math.floor(
                self.state.l_response * (self.state.potential_pop + decision.enroll)
            )
            # implements new patients and success process using Monte Carlo sampling
            p_true_samples = np.random.beta(
                self.state.success, self.state.failure, self.initial_state["K"]
            )
            MC_samples = mc_success_fn(
                decision.enroll,
                r_bar,
                p_true_samples,
            ...
            return {
                "new_patients": MC_samples["mc_enroll"],
    ...
    def transition_fn(self, decision, exog_info):
        ...
        """
        enroll_pop = decision.prog_continue * (
            self.state.potential_pop + decision.enroll
        )
        new_lambda = (
            1 - self.initial_state["alpha"]
        ) * self.state.l_response + self.initial_state["alpha"] * exog_info[
            "new_patients"
        ] / (
            self.state.potential_pop + decision.enroll
        )
        new_succ = self.state.success + exog_info["succ_count"]
        new_fail = self.state.failure + (
            exog_info["new_patients"] - exog_info["succ_count"]
        )
        return {
            "potential_pop": enroll_pop,
            "success": new_succ,
            "failure": new_fail,
...
if __name__ == "__main__":
    ...
    while t <= initial_state["trial_size"] and stop == False:
        ...
        print(
            "t={}, obj={}, state.potential_pop={}, state.success={}, state.failure={}, x={}".format(
                t,
                M.objective,
    ...
    print(
        "t={}, obj={}, state.potential_pop={}, state.success={}, state.failure={}, x={}".format(
            t, M.objective, M.state.potential_pop, M.state.success, M.state.failure, x
        )
    )

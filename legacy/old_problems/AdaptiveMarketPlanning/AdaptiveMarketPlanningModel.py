...
class AdaptiveMarketPlanningModel:
    ...
    def transition_fn(self, decision, exog_info):
        ...
        derivative = (
            ...
            if self.state.order_quantity < exog_info["demand"]
            else -self.cost
        )
        # update order quantity
        new_order_quantity = max(
            0, self.state.order_quantity + decision.step_size * derivative
        )
        ...
        new_counter = (
            ...
            if self.past_derivative * derivative < 0
    ...
    def step(self, decision):
        ...
        print(
            "t {}, Price {}, Demand {}, order_quantity {}, contribution {}".format(
                self.t,
                self.price,
                exog_info["demand"],
                self.order_quantity,
                onestep_contribution,
            )
        )

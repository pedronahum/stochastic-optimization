...
class AdaptiveMarketPlanningPolicy:
    ...
    def harmonic_rule(self):
        return self.M.build_decision(
            {"step_size": self.theta_step / (self.theta_step + self.M.t - 1)}
        )
    ...
    def kesten_rule(self):
        return self.M.build_decision(
            {
                "step_size": self.theta_step
                / (self.theta_step + self.M.state.counter - 1)
            }
        )
    ...
    def run_policy(self):
        ...
        for t in range(model_copy.T):
            model_copy.step(
                AdaptiveMarketPlanningPolicy(model_copy, self.theta_step).kesten_rule()
            )

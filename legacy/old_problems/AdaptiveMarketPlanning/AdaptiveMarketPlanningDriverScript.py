...
if __name__ == "__main__":
    ...
    M = AdaptiveMarketPlanningModel(
        state_names, decision_names, init_state, T, reward_type, price, cost
    )
    ...
    print(
        "Optimal order_quantity for price {} and cost {} is {}".format(
            price, cost, optimal_order_quantity
        )
    )
    print(
        "Reward type: {}, theta_step: {}, T: {} - Average reward over {} iteratios is: {}".format(
            reward_type, theta_step, T, trial_size, rewards_per_iteration_cum_avg[-1]
        )
    )
    ...
    fig1.suptitle(
        "Reward type: {}, theta_step: {}, T: {}".format(reward_type, theta_step, T)
    )
    ...
    plt.plot(
        time, order_quantity, label="Kesten's Rule for theta_step {}".format(theta_step)
    )
    plt.legend()
    plt.show()

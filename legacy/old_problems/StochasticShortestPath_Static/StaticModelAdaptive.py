...
class StaticModel:
    ...
    def __init__(self, state_names, x_names, params):
        ...
        self.g, self.V_t, self.origin_node, self.target_node, self.dist = (
            self.createStochasticGraph()
        )
        self.init_args.update(
            {"start_node": self.origin_node, "target_node": self.target_node}
        )
    ...
    def createStochasticGraph(self):
        ...
        g = randomgraphChoice(
            self.prng,
            self.init_args["nNodes"],
            self.init_args["probEdge"],
            self.init_args["LO_UPPER_BOUND"],
            self.init_args["HI_UPPER_BOUND"],
        )

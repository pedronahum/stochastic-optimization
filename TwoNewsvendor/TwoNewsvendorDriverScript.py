...
from TwoNewsvendorLearning import Choice, Learning_model_central, Learning_model_field
from TwoNewsvendorPolicy import (
    Policy_Central,
    Policy_Field,
    create_theta_grid,
    plot_heat_map,
...
from TwoNewsvendor import Exogenous_Information, Model_Central, Model_Field
...
def Main():
    ...
    for theta_field, theta_central in theta_grid:
        ...
        print(
            "\nStarting!!!!!  policy_field: {} with par_field: {}  - policy_central: {} with par_central {}".format(
                params["policy_field"],
                theta_field,
                params["policy_central"],
                theta_central,
            )
        )

        if "learning" in params["policy_field"]:
            M_field = Learning_model_field(
                theta_field,
                state_names_field,
                decision_names_field,
                state_field,
                params,
            )
        else:
            M_field = Model_Field(
                state_names_field, decision_names_field, state_field, params
            )

        P_field = Policy_Field(params, theta_field)

        if "learning" in params["policy_central"]:
            M_central = Learning_model_central(
                theta_central,
                state_names_central,
                decision_names_central,
                state_central,
                params,
            )
        else:
            M_central = Model_Central(
                state_names_central, decision_names_central, state_central, params
            )
        ...
        avg_cum_cost_field[(theta_field, theta_central)] = list(
            np.divide(accum_cost_field, params["n_Testing_Ite"])
        )
        avg_cum_cost_central[(theta_field, theta_central)] = list(
            np.divide(accum_cost_central, params["n_Testing_Ite"])
        )
        avg_cum_cost_company[(theta_field, theta_central)] = list(
            np.divide(accum_cost_company, params["n_Testing_Ite"])
        )

        avg_total_cost_field[(theta_field, theta_central)] = avg_cum_cost_field[
            (theta_field, theta_central)
        ][-1]
        avg_total_cost_central[(theta_field, theta_central)] = avg_cum_cost_central[
            (theta_field, theta_central)
        ][-1]
        avg_total_cost_company[(theta_field, theta_central)] = avg_cum_cost_company[
            (theta_field, theta_central)
        ][-1]

        print(
            "Finishing iterations for theta ({},{}). Avg reward for field: {:.2f}, avg reward for central {:.2f} and avg reward for company {:.2f}".format(
                theta_field,
                theta_central,
                avg_cum_cost_field[(theta_field, theta_central)][-1],
                avg_cum_cost_central[(theta_field, theta_central)][-1],
                avg_cum_cost_field[(theta_field, theta_central)][-1]
                + avg_cum_cost_central[(theta_field, theta_central)][-1],
            )
        )
        ...
        print(
            "Average request from field {:.2f} - Average allocated quantity by central {:.2f}".format(
                avg_request_field / params["n_Testing_Ite"],
                avg_allocated_central / params["n_Testing_Ite"],
            )
        )
    ...
    dfTest = dfOutputPath.pivot_table(
        ["field_dec_" + s for s in decision_names_field]
        + ["central_dec_" + s for s in decision_names_central]
        + [
            "central_state_field_weight",
            "central_state_source_weight",
            "exog_est_field",
            "exog_est_central",
            "exog_demand",
        ],
    ...
    field_sorted_by_value = sorted(
        avg_cum_cost_field.items(), key=lambda kv: kv[1][-1], reverse=True
    )
    central_sorted_by_value = sorted(
        avg_cum_cost_central.items(), key=lambda kv: kv[1][-1], reverse=True
    )
    company_sorted_by_value = sorted(
        avg_cum_cost_company.items(), key=lambda kv: kv[1][-1], reverse=True
    )

    selected_field_positions = [
        i
        for i, dict_entry_field in enumerate(field_sorted_by_value)
        if dict_entry_field[0]
        in [
            central_sorted_by_value[0][0],
            company_sorted_by_value[0][0],
            central_sorted_by_value[-1][0],
            company_sorted_by_value[-1][0],
        ]
    ]
    ...
    displaySet = (
        [0]
        + list(
            prng.choice(np.arange(1, len(field_sorted_by_value) - 1), p, replace=False)
        )
        + [len(field_sorted_by_value) - 1]
    )

    Rounds = list(range(params["N"]))

    # Preparation to plot the added bias
    dfBias = dfOutputPath.pivot_table(
        ["field_dec_bias_applied", "central_dec_bias_applied"],
        index=["theta_field_theta_central", "n"],
        columns=["ite"],
    )
    ...
    if params["print_records"]:
        ...
        writer = pd.ExcelWriter(
            "DetailedOutput_{}_{}.xlsx".format(
                params["policy_field"], params["policy_central"]
            ),
            engine="xlsxwriter",
        )
        ...
        print(
            "Finished printing the excel file. Elapsed time {}\n".format(
    ...
    plot_heat_map(
        ...
        theta_central_list,
        "Company",
        company_sorted_by_value,
    )
    plt.suptitle(
        "Averaged total rewards after {} time periods ({} samples) \n policy_field: {} - policy_central: {}".format(
            params["N"],
            params["n_Testing_Ite"],
            params["policy_field"],
            params["policy_central"],
        )
    )
    ...
    figBias.suptitle(
        "Bias applied by field and central along time periods for 5 different sample paths \n policy_field: {} - policy_central: {}".format(
            params["policy_field"], params["policy_central"]
        )
    )
    ...
    i = -1
    for grid_index in eval(
        "{}_{}".format(params["policy_field"], params["policy_central"])
    ):
        i += 1
        theta = "{}_{}".format(theta_grid[grid_index][0], theta_grid[grid_index][1])

        testnpField = dfBias.loc[
            idx[theta, 0 : params["N"]],
            idx["field_dec_bias_applied", list(order[grid_index, :])],
        ].values
        testnpCentral = dfBias.loc[
            idx[theta, 0 : params["N"]],
            idx["central_dec_bias_applied", list(order[grid_index, :])],
        ].values
        ...
        axBias[i].set_title(
            " Field_Central parameter pair ({},{})".format(
                theta_grid[grid_index][0], theta_grid[grid_index][1]
            ),
            fontsize=10,
        )
    ...
    figHist.suptitle(
        "Histogram of the bias applied by field and central along all sample paths \n policy_field: {} - policy_central: {}".format(
            params["policy_field"], params["policy_central"]
        )
    )
    ...
    for grid_index in eval(
        ...
        arrayBiasField = dfBias.loc[
            idx[theta, 10 : params["N"]], idx["field_dec_bias_applied", :]
        ].values.reshape(1, -1)
        histF = np.histogram(arrayBiasField, rangeF)

        arrayBiasCentral = dfBias.loc[
            idx[theta, 10 : params["N"]], idx["central_dec_bias_applied", :]
        ].values.reshape(1, -1)
        histC = np.histogram(arrayBiasCentral, rangeC)

        axHist[i].bar(
            histF[1][0 : len(rangeF) - 1], histF[0] / sum(histF[0]), label="Field"
        )
        axHist[i].bar(
            histC[1][0 : len(rangeC) - 1], histC[0] / sum(histC[0]), label="Central"
        )
        axHist[i].legend()
    ...
    figPath.suptitle(
        "Avg Rewards and quantities along rounds \n policy_field: {} - policy_central: {}".format(
            params["policy_field"], params["policy_central"]
        )
    )
    ...
    p = axsubs[1, 2].plot(
        Rounds,
        dfTest.loc["{}_{}".format(theta_field, theta_central), :]["exog_demand"],
        linestyle="dotted",
        label="Avg Demand",
    )
    axsubs[1, 2].plot(
        Rounds,
        dfTest.loc["{}_{}".format(theta_field, theta_central), :]["exog_est_field"],
        color=p[-1].get_color(),
        linestyle="dashed",
        label="Avg Source Estimate to Field",
    )
    axsubs[1, 2].plot(
        Rounds,
        dfTest.loc["{}_{}".format(theta_field, theta_central), :]["exog_est_central"],
        color=p[-1].get_color(),
        label="Avg Source Estimate to Central",
    )
    ...
    for dict_entry, rank in zip(
        field_sorted_by_value, range(len(field_sorted_by_value))
    ):
        theta_field, theta_central = dict_entry[0][0], dict_entry[0][1]

        rank_central = [
            i
            for i, dict_entry_central in enumerate(central_sorted_by_value)
            if dict_entry_central[0] == dict_entry[0]
        ]
        rank_company = [
            i
            for i, dict_entry_company in enumerate(company_sorted_by_value)
            if dict_entry_company[0] == dict_entry[0]
        ]
        ...
        print(
            "({:.2f},{:.2f}) -  Agents rewards ({:.2f},{:.2f}) and company reward {:.2f} - rank_field {}, rank_central {}, rank_company {}".format(
                theta_field,
                theta_central,
                c_f,
                c_c,
        ...
        if "two_estimates" in params["policy_central"] and theta == maxVarIndex:
            ...
            axWeight.plot(
                dfTest.loc[theta, :]["central_state_field_weight"],
                label="Weight to estimate from field",
            )
            axWeight.plot(
                dfTest.loc[theta, :]["central_state_source_weight"],
                label="Weight to estimate from source",
            )
            axWeight.legend()

            axWeight.set_title(
                r"Central weights when combining two sources of information for $(\theta^{q},\theta^{q'})=$"
                + "({},{})".format(theta_field, theta_central)
            )
        ...
        if rank in displaySet or rank in selected_field_positions:
            ...
            axsubs[0, 0].plot(
                Rounds,
                np.array(avg_cum_cost_field[(theta_field, theta_central)]),
                linestyle="dashed",
                label="({:.2f},{:.2f} - {})".format(theta_field, theta_central, rank),
            )

            # Central cost
            axsubs[0, 1].plot(
                Rounds,
                np.array(avg_cum_cost_central[(theta_field, theta_central)]),
                label="({:.2f},{:.2f} - {})".format(
                    theta_field, theta_central, rank_central[0]
                ),
            )
            ...
            axsubs[0, 2].plot(
                Rounds,
                np.array(avg_cum_cost_company[(theta_field, theta_central)]),
                label="({:.2f},{:.2f} - {})".format(
                    theta_field, theta_central, rank_company[0]
                ),
            )
            ...
            axsubs[1, 0].plot(
                Rounds,
                dfTest.loc["{}_{}".format(theta_field, theta_central), :][
                    "field_dec_quantity_requested"
                ],
                linestyle="dashed",
                label="({},{} - {})".format(theta_field, theta_central, rank),
            )
            ...
            axsubs[1, 1].plot(
                Rounds,
                dfTest.loc["{}_{}".format(theta_field, theta_central), :][
                    "central_dec_quantity_allocated"
                ],
                label="({},{} - {})".format(
                    theta_field, theta_central, rank_central[0]
                ),
